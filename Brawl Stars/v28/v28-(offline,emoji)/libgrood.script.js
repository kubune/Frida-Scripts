...
function OfflineBattles() {
        Interceptor.attach(cache.base.add(0x5DF3EC), { // BattleScreen::shouldShowChatButton
		    onLeave(retval) {
			    retval.replace(1);
		    }
     	});
     	
        var StartGamePtr = cache.base.add(START_GAME);
        
        Interceptor.attach(StartGamePtr, { //Offline battles
          onEnter: function(args) {
            args[3] = ptr(3);
            args[6] = ptr(1);
            
            //0x23B004 CombatHUD::getChatButton
...
rpc.exports = {
    init: function(stage, options) {
        ...
        onLoad('libg.so', function() {
            ...
            setup();
            toast("Lwitchy's Brawl v28");
            enableDebugInfo();
            packageName('com.groodx.brawl');
...
/*kullanışsız
            ...
            var Cemera = cache.base.add(0xC27E4C);
            Memory.protect(Cemera, 1, "rwx")
            Cemera.writeInt(0);
            
            var ArtTest = cache.base.add(0xC1B045);
            Memory.protect(ArtTest, 1, "rwx")
            ArtTest.writeInt(0);
